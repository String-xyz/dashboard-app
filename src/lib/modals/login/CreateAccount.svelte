...
<ModalBase>
	<div class='main flex flex-col items-center'>
		...
		<StyledInput className="mb-6 w-full" label="Your Full Name" placeholder="Ex. John Doe" bind:value={fullNameInput} />
		<StyledInput className="mb-6 w-full" label="Company Name" placeholder="Enter Company name" bind:value={companyNameInput} />
		<StyledInput className="mb-8 w-full" label="Email" placeholder="Enter email" bind:value={emailInput} />

		<StyledButton className="w-full" {disabled} action={createAccount}>Continue</StyledButton>
		<!-- svelte-ignore a11y-click-events-have-key-events -->
		<span>Have an account? <span on:click={() => $activeModal = Login} class="link link-primary mt-2">Login</span></span>
	</div>
</ModalBase>
